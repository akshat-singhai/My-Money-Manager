...
const TransactionList = () => {
  ...
  // Filter + sort: recent transactions first (by date desc)
const filtered = useMemo(() => {
  ...
  const list = (Array.isArray(transactions) ? transactions : []).filter((tx = {}) => {
    ...
    // Add support for cash/account filtering by paymentMode
    if (filterType === "cash") {
      matchType = tx.paymentMode === "cash";
    } else if (filterType === "account") {
      matchType = tx.paymentMode === "account";
    }
  ...
  const handleDownload = () => {
    ...
    const headers = ["Date", "Description", "Amount", "Type", "Category", "PaymentMode", "Notes"];
